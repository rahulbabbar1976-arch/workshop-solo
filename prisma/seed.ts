...
async function main() {
  console.log('Seeding database via TSX...');
  ...
  const ownerUser = await prisma.user.create({
    data: {
      ...
      quickPinHash: hashPassword('0000'),
      isActive: true,
      roles: {
        create: {
          roleId: seededRoles['accounts'].id,
          isPrimary: true
        }
      }
    }
  });
  console.log(`Owner user seeded: ${ownerUser.fullName}`);

  // Create Default Parts Manager User for Testing
  const storeUser = await prisma.user.create({
    data: {
      fullName: 'Store Parts Manager',
      mobile: '5555555555',
      email: 'store@workshop.local',
      passwordHash: hashPassword('store123'),
      quickPinHash: hashPassword('7777'),
      isActive: true,
      roles: {
        create: {
          roleId: seededRoles['parts_manager'].id,
          isPrimary: true
        }
      }
    }
  });
  console.log(`Parts Manager user seeded: ${storeUser.fullName}`);
  ...
  for (const i of icons) {
    await prisma.complaintIconMaster.upsert({
      where: { iconKey: i.iconKey },
      update: {},
      create: i
    });
  }
