...
class ANPRPipeline:
    ...
    def process_frame(self, frame: np.ndarray,
        ...
        if not boxes and self.detector_kind == "classic":
            try:
                full_results = self.reader.reader.readtext(frame)
        ...
        for (x, y, w, h) in boxes:
            candidates = []
            # Try multiple paddings to get the best crop
            for pad_factor in [0.05, 0.15, 0.25]:
        ...
        annotated = self._annotate(frame, unique_results)
