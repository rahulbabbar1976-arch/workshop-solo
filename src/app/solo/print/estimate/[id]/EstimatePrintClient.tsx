...
export function EstimatePrintClient({ estimate, workshopProfile }: { estimate: any, workshopProfile?: any }) {
  ...
  let activeParts = [];
  try {
    activeParts = estimate.partsSnapshot ? JSON.parse(estimate.partsSnapshot) : [];
  } catch (e) {}

  let activeLabor = [];
  try {
    activeLabor = estimate.laborSnapshot ? JSON.parse(estimate.laborSnapshot) : [];
  } catch (e) {}
  ...
  const services = (activeLabor).map((l: any, i: number) => {
  ...
  const products = (activeParts).map((p: any, i: number) => {
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col md:flex-row">
      ...
      <div className={`print:hidden bg-white shadow-lg border-r border-gray-200 transition-all duration-300 ${isSettingsOpen ? 'w-full md:w-64 p-6' : 'w-0 overflow-hidden'}`}>
        <div className="flex justify-between items-center mb-6">
          ...
          <Link href={`/solo/jobcards/${estimate.jobCardId}`} className="text-teal-600 hover:text-teal-700">
