...
// 2. Replace labor button with FAB
// we will do a regex to replace "Add Labor" button
code = code.replace(
...
// 3. Add pictures done icon
// Just add it before the very last </div> if it's not there.
const picturesFAB = `
      {activeTab === "pictures" && (
        <button 
...
if (!code.includes('onClick={() => setActiveTab("details")}')) {
  // Let's insert it right after the global style
  code = code.replace(
    /<div className="stage">/,
    `<div className="stage">\n${picturesFAB}`
  );
